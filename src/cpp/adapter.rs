...
impl<'a> LexerAdapter<'a> for CppLexerAdapter {
    fn tokenize(&self, input: &'a str) -> Vec<LocatedToken<'a>> {
        ...
        let mut offset = 0;
        let mut result = Vec::with_capacity(cpp_tokens.len() + 1);

        for token in cpp_tokens.iter() {
        ...
        let eof_token = Token::eof(input);
        let (line, column) = {
            // Compute line/column for EOF position
            let end = eof_token.start;
            let prefix = &input[..end.min(input.len())];
        ...
        let start = eof_token.start;
        let end = eof_token.end;
        result.push(LocatedToken {
            token: eof_token,
            span: Span {
                start,
